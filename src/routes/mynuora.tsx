...
export const Route = createFileRoute("/mynuora")({
  component: PDP,
  head: () => ({
    meta: [
...
function PDP() {
  return (
    ...
    <main className="mynuora-theme min-h-screen bg-background text-foreground pb-24">
        ...
        <div className="space-y-4 min-w-0 -mt-2 lg:mt-0 pt-2">
          ...
          <div className="grid grid-cols-2 gap-2 -mt-1">
            {BENEFITS.map((b, i) => (
              <div
                key={i}
                className="flex items-start gap-2 rounded-lg border border-border bg-card px-2.5 py-2 h-full hover:bg-muted/40 transition-colors"
              >
                <div className="w-5 h-5 mt-0.5 rounded-full bg-rose-deep/10 flex items-center justify-center flex-shrink-0 ring-1 ring-rose-deep/20">
                  <Check className="w-3 h-3 text-rose-deep" strokeWidth={3.5} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] font-extrabold text-foreground leading-[1.2] tracking-tight">
                    {b.title}
                  </div>
                  <div className="text-[10px] text-muted-foreground leading-[1.25] mt-0.5">
                    {b.subtitle}
                  </div>
                </div>
              </div>
      ...
      {/* SECTION 15 — Doctor Advisory Board */}
      <DoctorBoard />

      {/* SECTION 15b — 99% Pure. Certified by Analysis. */}
      <PurityCertified />
